...
import { Edit2, Trash2, Users, Clock, CalendarRange } from 'lucide-react';
...
export function PtPackagesTable({ onEdit }: PtPackagesTableProps) {
    ...
    const handleDelete = (item: PtPackage) => {
        toast(`Remove "${item.name}"?`, {
            description: 'This will permanently delete this package record.',
            duration: 8000,
    ...
    const columns = [
        ...
        columnHelper.accessor('durationMinutes', {
            ...
            cell: (info) => (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-white font-bold text-sm">{info.getValue()} min</span>
                </div>
            ),
        }),
        columnHelper.accessor('validityDays', {
            header: 'VALIDITY',
            cell: (info) => (
                <span className="text-[var(--text-secondary)] font-bold text-sm">{info.getValue()} days</span>
            ),
        }),
        columnHelper.accessor('defaultAmount', {
            ...
            cell: (info) => (
                <span className="font-black text-[var(--primary)] text-sm">
                    LKR {info.getValue().toLocaleString()}
                </span>
            ),
        }),
                        ...
                        {table.getRowModel().rows.length === 0 ? (
                            <tr>
                                <td colSpan={columns.length} className="px-6 py-16 text-center text-[var(--text-tertiary)] text-sm italic">
                                    No packages registered yet. Click <span className="font-bold text-white">REGISTER PACKAGE</span> to add one.
                                </td>
                            </tr>
                        ) : (
