...
import { rolesApi, type AppRoleDto, type RolePermissionDto } from '../api/roles-api';

// ═══════════════════════════════════════════════════════════════
// Types & Constants
// ═══════════════════════════════════════════════════════════════
type View = 'hub' | 'role-list' | 'role-editor';
...
// ═══════════════════════════════════════════════════════════════
// Hub View — two-card dashboard
// ═══════════════════════════════════════════════════════════════
function HubView({ onGoToRoles }: { onGoToRoles: () => void }) {
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="relative bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-8 overflow-hidden opacity-50 select-none">
                    <span className="absolute top-4 right-4 px-2.5 py-1 rounded-lg bg-zinc-700/60 border border-zinc-600/30 text-zinc-400 text-[9px] font-black uppercase tracking-widest">
                        Coming Soon
                    </span>
                    ...
                    <p className="text-[var(--text-tertiary)] text-sm mb-6 leading-relaxed">
                        Create staff accounts and assign roles to control their access levels across the system.
                    </p>
                    <div className="space-y-2">
                        {['Create staff logins', 'Assign roles to users', 'Enable / disable accounts'].map(f => (
                ...
                >
                    ...
                    <p className="text-[var(--text-tertiary)] text-sm mb-6 leading-relaxed">
                        Define roles and set L1–L5 access levels for every section of the app.
                    </p>
                    <div className="space-y-2 mb-6">
                        {[
                            ['text-[var(--success)]', 'Create custom roles'],
                            ['text-[var(--success)]', 'Set L1–L5 access per section'],
...
function RoleListView({ onBack, onEditRole }: RoleListViewProps) {
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Roles list */}
            {isLoading ? (
                <div className="flex items-center justify-center h-32 text-[var(--text-tertiary)] text-sm animate-pulse">Loading roles…</div>
            ) : (
                            ...
                            >
                                <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                        ...
                                        {isOwner && (
                                            <p className="text-[10px] text-amber-400/70 font-bold flex items-center gap-1"><Lock className="w-3 h-3" /> Full Access — Locked</p>
                                        )}
                                    </div>
                                </div>
...
// ═══════════════════════════════════════════════════════════════
// Role Permission Editor — L1-L5 per section
// ═══════════════════════════════════════════════════════════════
...
function RolePermEditor({ roleName, onBack }: RolePermEditorProps) {
    ...
    return (
        <div className="space-y-8">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[var(--text-tertiary)] text-xs font-bold mt-0.5">
                                {isOwner ? 'Full access to all sections — cannot be restricted' : 'Click a level button to change access for each section'}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            {/* Permission matrix */}
            {!isOwner && isLoading ? (
                <div className="flex items-center justify-center h-32 text-[var(--text-tertiary)] text-sm animate-pulse">Loading permissions…</div>
            ) : (
...
// ═══════════════════════════════════════════════════════════════
// Main Component — manages internal navigation
// ═══════════════════════════════════════════════════════════════
        ...
        <AnimatePresence mode="wait">
            {view === 'hub' && (
                <motion.div key="hub" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                    <HubView onGoToRoles={() => setView('role-list')} />
                </motion.div>
            )}
            {view === 'role-list' && (
